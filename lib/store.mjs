...
/* Key prefixes that live in the database and must come back on a fresh host.
   `sites/` is the per-site content; `team/` is the member list — without it a
   redeploy would lock the whole team out. Keep in sync with `mirrorable()` in
   server.mjs, which decides what gets written up in the first place. */
export const PERSISTED_PREFIXES = ['sites/', 'team/'];

/* Pull every persisted doc from Mongo down to the local fs cache (used on a host
...
export async function hydrateToFs() {
  ...
  const re = '^(' + PERSISTED_PREFIXES.map(reEsc).join('|') + ')';
  const docs = await store.__coll.find({ _id: { $regex: re } }).toArray();
  let n = 0;
  for (const d of docs) {
...
async function seedFromFs(target) {
  ...
  for (const p of PERSISTED_PREFIXES) await walk(p.replace(/\/$/, ''));
