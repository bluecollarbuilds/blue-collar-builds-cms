...
/* Attributes that point at something the page needs in order to render. `script` is
   absent on purpose: autotag strips scripts, so a captured page never needs them. */
const ASSET_ATTRS = [
  ['link[href]', 'href'],
  ['img[src]', 'src'],
  ['img[srcset]', 'srcset'],
...
export function assetPathOf(url, origin) {
  let u;
 ...
 */
export function collectHtmlAssets($, origin) {
  const found = new Map();                              // absolute url -> asset path
  const seen = [];
  ...
  for (const [sel, attr] of ASSET_ATTRS) {
    $(sel).each((_, el) => {
      ...
      if (!raw) return;
      const note = (u) => { const p = assetPathOf(u, origin); if (p) found.set(u, p); return u; };
 ...
 */
export function collectCssAssets(css, cssUrl, origin) {
  const found = new Map();
